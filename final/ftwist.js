...
function tickBoard(time)
{
	...
	if (board.timeAllowed<0) {
		...
		var best=parseInt(localStorage.getItem('best'+board.goalTime));	
	    if (isNaN(best)) best=0;
		if (board.score>best) localStorage.setItem('best'+board.goalTime,board.score);
	
		document.getElementById('completestatus').innerHTML="Final Score "+board.score+"<p/>(Previous best "+best+")";		
